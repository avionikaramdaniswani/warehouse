        ...
        <CardContent className="p-0">
          ...
          <div className="md:hidden flex flex-col gap-3 p-3">
            {lowStockItems.slice(0, 5).map((item) => {
              ...
              const pct = Math.min(100, Math.round((item.stok / (item.safetyStok * 2)) * 100));
              const accent = isHabis ? 'border-red-500 bg-red-50/60' : 'border-amber-400 bg-amber-50/40';
              const barColor = isHabis ? 'bg-red-500' : 'bg-amber-400';
              const numColor = isHabis ? 'text-red-600' : 'text-amber-600';
              return (
                <div key={item.tsCode} className={`rounded-xl border-l-4 border border-slate-100 shadow-sm ${accent} p-4`}>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="min-w-0">
                      <p className="font-semibold text-sm text-slate-800 leading-snug line-clamp-2">{item.nama}</p>
                      <p className="text-xs font-mono text-slate-400 mt-0.5">{item.tsCode} · {item.kategori}</p>
                    </div>
                  ...
                  <div className="flex items-end justify-between mt-3 gap-3">
                    <div className="flex-1">
                      <div className="flex justify-between text-xs text-slate-400 mb-1">
                        ...
                        <span>Min {item.safetyStok} {item.uom}</span>
                      </div>
                      <div className="h-2 w-full bg-slate-200 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all ${barColor}`} style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                    <p className={`text-2xl font-bold leading-none shrink-0 ${numColor}`}>
                      {item.stok} <span className="text-xs font-normal text-slate-400">{item.uom}</span>
                    </p>
                  </div>
                </div>
              );
