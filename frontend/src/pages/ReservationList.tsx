...
export default function ReservationList() {
  ...
  const handleSimpan = async () => {
    ...
    try {
      ...
      resetForm();
      await fetchData();
      const newest = (await fetch('/api/transaksi-keluar', {
        headers: { Authorization: `Bearer ${token}` },
      }).then(r => r.json()) as TransaksiKeluar[]).find(t => t.nomor === result.nomor);
    ...
    <Layout title="Reservation List">
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          ...
          {canEdit && (
            <Button className="w-full sm:w-auto h-9" onClick={() => { resetForm(); setFormOpen(true); }}>
      ...
      {/* Form Dialog — Buat Reservation List Baru */}
